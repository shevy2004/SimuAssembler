...
{
    char* temp = cp;
    if (*temp == '.' && (!strncmp(temp + 1, "data", DATA_LEN) || !strncmp(temp + 1, "string",STRING_LEN) || !strncmp(temp + 1, "entry", ENTRY_LEN ) ||
        !strncmp(temp + 1, "extern", EXTERN_LEN)))
        return 1;
    return 0;
}
...
void binary(short binar, instructionsTable** insteruction_head)
{
    ...
    addInstructions(insteruction_head, word);
}
...
void set_binary(short binar, instructionsTable** insteruction_head, int back)
{
    ...
    short mask1 = 1 << (WORD_LEN-1);
    int i;
    int length = num_of_cells(*insteruction_head);
    for (i = 0; i < WORD_LEN; i++)
    {
        if (mask1 & binar)
            word[i] = '1';
    ...
    setInstructions(*insteruction_head, word, length - back);
}
...
void data_code(int arr[], int length, dataTable** data_head)
{
    ...
    for (i = 0; i < length; i++)
    {
        binary_data((short)arr[i], data_head);
    }
...
void string_code(char* string, dataTable** data_head)
{
    ...
    {
        binary_data((short)string[i], data_head);
    }
    binary_data(0, data_head);
}
...
int update_adress_label(char label_name[LABEL_LENGTH], labelTable* head, int count_line, instructionsTable** insteruction_head, int L)
{
    ...
    {
        int i;
        int adress = find_adress_label(head, label_name);
        int length = num_of_cells(*insteruction_head);   
