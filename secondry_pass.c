	...
	char help1_label_name[LABEL_LENGTH] = { "\0" };
        ...
        if (*line != ';')
        {
            ...
            { /*If you have reached the name of a function*/
                ...
                { /*If you did find the function name in the function array*/
                        ...
                        {  /* else 1*/
                            ...
                            if (update_adress_label(label_name, head, count_line, &insteruction_head, IC) == 1)   /*Checks that the label exists in the symbol table*/
                            {
                                ...
                                if (is_extern(head, help_label_name)) /*Checks if the label is external*/
                                {
                                    ...
                                    fprintf(ext_file, "%s\t%d\n", help_label_name, IC + FIRST_LINE_NUMBER); 
                                }
                                count_len_label = 0;
                    ...
                    else/*if it is one parameter */
                    {
                        if (number_func == NUM_JMP || number_func == NUM_BNE || number_func == NUM_JSR)/*if it is jump*/
                        {
                            ...
                            while (isspace(*cp))
                                cp++;
			    if ((*cp) == '*' && cp[1] == 'r' && cp[2] >= '0' && cp[2] <= '7' && !isdigit(cp[3])) /*If it is a *register*/
                            {
			        cp = cp + 3;
                                while (isspace(*cp))
                                    cp++;
			    ...
			    else if ((*cp) == '*' && cp[1] == 'r' && cp[2] >= '0' && cp[2] <= '7' && !isdigit(cp[3])) /*If it is a non direct register*/
                            {
			        cp += 3;
                                while (isspace(*cp))
                                    cp++;
                            ...
                            if (flag_number == 0) /*if it's a label*/
                            { 
                                while (((isalpha(*cp) || isdigit(*cp))) && flag_number == 0)
                                {
                                    cp++;
...
int three_bit_binary_to_decimal(const char *three_bit_binary)
{
    int decimal = 0;
    int i;
    /* Convert three-bit binary string to decimal */
    for (i = 0; i < 3; ++i) 
    {
        if (three_bit_binary[2 - i] == '1') 
        {
            decimal += 1 << i;  /* Calculate decimal value using bitwise left shift */
        }
    } 
    return decimal;
}
...
int *binary_15bit_to_decimals(const char *binary_15bit)
{
    ...
    /* Convert 15-bit binary string to array of decimals */
    for (i = 0; i < 5; ++i) 
    {
        char three_bit_binary[3]; 
        /* Copy next 3 bits from binary_15bit to three_bit_binary */
        strncpy(three_bit_binary, &binary_15bit[i * 3], 3);
        
        /* Convert three_bit_binary to decimal and store in decimal_array1 */
